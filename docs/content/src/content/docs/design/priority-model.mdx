...
That question becomes more important when a project customizes the builder.
Custom chain methods can introduce states like `pressed`, `open`, `selected`,
or named breakpoints. Those methods still need predictable behavior in runtime
mode and in extracted CSS.
...
The generated CSS should not depend on whether this value was resolved by the
runtime or extracted by the compiler. The same authored chain needs the same
cascade path.
