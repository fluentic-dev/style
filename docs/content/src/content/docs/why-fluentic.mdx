...
But a component style theme where each public part can be composed explicitly,
without consumers reaching into DOM structure or relying on class names as
hidden hooks, with something as simple as this:
...
A consumer might need to customize those parts without depending on DOM structure.
...
But now the consumer knows your class names.
